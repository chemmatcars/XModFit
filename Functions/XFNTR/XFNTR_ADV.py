    ...
    def fluCalFun(self, x, topdel, topbet, botdel, botbet, flutopdel, flutopbet, flubotdel, flubotbet, flutopmu, flubotmu, topmu, k0, conbot, contop):
        ...
        for i in range(len(x)):
           ...
           # print(z_imed-z_i1)
            ...
            tot_top = self.yscale * stepsize * contop * np.sum(np.where(xprime > -self.samplesize * 1e7 / 2, int_top * weighthit, 0)) * self.__avoganum__ / 1e27
            tot_bot = self.yscale * stepsize * conbot * np.sum(np.where(xprime > -self.samplesize * 1e7 / 2, int_bot * weighthit, 0)) * self.__avoganum__ / 1e27

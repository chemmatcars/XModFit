...
class SymSphere: #Please put the class name same as the function name
    def __init__(self,x = 0.1, E = 10.0, R0 = 25.00, rhoc = 4.68, D = 66.6, rhosh = 0.200, h1 = -25.0, h1sig = 0.0, h2 = 3.021,
                 ...
                 mpar={'AdLayer':{'Layers':['top', 'bottom'], 'd':[0.0,1.0],'rho':[0.0,0.334],'mu':[0.0,0.0],'sig':[0.0,3.00]}},
        ...
        mpar      : The monolayer parameters where,
    ...
    @lru_cache(maxsize=10)
    def calcProfile2(self,d,rho,mu,sig,zmin,zmax,dz):
        """
        ...
        __mu2__ = self.sldCalFun(d, tuple(mu), tuple(sig), tuple(__z2__))
        return __z2__-np.sum(d[1:-1]),__d2__,__rho2__,__mu2__
    ...
    def update_parameters(self):
        ...
        self.__mu__ = tuple(
            [self.params['__%s_mu_%03d' % (mkey, i)].value for i in range(len(self.__mpar__[mkey]['mu']))])
        self.__sig__ = tuple(
    ...
    def y(self):
        ...
        z2,d2,rho2,mu2=self.calcProfile2(self.__d__,self.__rho__,self.__mu__,self.__sig__,self.zmin,self.zmax,self.dz)
        refq2,r2=self.py_parratt(tuple(x),lam,tuple(d2),tuple(rho2),tuple(mu2))
        ...
        if not self.__fit__:
            self.output_params['Nanoparticle_EDP'] = {'x': z1, 'y': rho1,
                                                      'names': ['z (Angs)', '\u03c1 (el/Angs^3)'],'plotType':'step'}
            self.output_params['Monolayer_EDP'] = {'x': z2, 'y': rho2, 'names':['z (Angs)','rho (el/Angs^3'],'plotType':'step'}
            self.output_params['Monolayer_ADP'] = {'x': z2, 'y': mu2, 'names':['z (Angs)','mu (1/cm)'],'plotType':'step'}

        if self.rrf:
            rhos1=(rho1[0],rho1[-1])
            mus1=(0,0)
            ref,r2=self.py_parratt(tuple(x-self.qoff),lam,(0.0,1.0),rhos1,mus1)
            refq=refq/ref
            crefq=crefq/ref
        if self.coherrent:
            return crefq

...
class XLayers_Triphasic: #Please put the class name same as the function name
    ...
    def y(self):
        ...
        if not self.coherrent:
            ...
            trefq=self.cov1*refq[mkeys[0]]+self.cov2*refq[mkeys[1]]+(1.0-self.cov1-self.cov2)*refq[mkeys[2]]
            if self.rrf:
                    ...
                    if not self.__fit__:
                        ...
                        self.output_params['%s_step_ADP' % mkey] = {'x': tz, 'y': tmu,
                                                                    'names': ['z (Angs)',
                                                                              'mu (1/cm)'],
                                                                    'plotType': 'step'}
                deld1=np.sum(self.__d__[mkeys[0]][:-1])-np.sum(self.__d__[mkeys[2]][:-1])
                deld2 = np.sum(self.__d__[mkeys[1]][:-1]) - np.sum(self.__d__[mkeys[2]][:-1])
                refq = abs(np.exp(-1j*self.x*deld1)*self.cov1 * r2[mkeys[0]] + np.exp(-1j*self.x*deld2)*self.cov2 * r2[mkeys[1]] + (1-self.cov1-self.cov2) * r2[mkeys[2]])**2
                    ...
                    if not self.__fit__:
                        ...
                        self.output_params['%s_step_ADP' % mkey] = {'x': tz, 'y': tmu,
                                                                    'names': ['z (Angs)',
                                                                              'mu (1/cm)'],
                                                                    'plotType': 'step'}
                trho=self.cov1*rho[mkeys[0]]+self.cov2*rho[mkeys[1]]+(1-self.cov1-self.cov2)*rho[mkeys[1]]
                tmu = self.cov1*mu[mkeys[0]]+self.cov2*mu[mkeys[1]]+(1-self.cov1-self.cov2)*rho[mkeys[1]]
                if not self.__fit__:

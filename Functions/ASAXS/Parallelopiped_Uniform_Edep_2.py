...
def parallelopiped_ml(q, L, B, H, rho, Nphi, Npsi):
    dphi=np.pi/Nphi
    dpsi=2.0*np.pi/Npsi
    fft = np.zeros_like(q)
    Nlayers=len(L)
...
class Parallelopiped_Uniform_Edep_2: #Please put the class name same as the function name
    def __init__(self, x=0, Np=10, error_factor=1.0, dist='Gaussian', Energy=None, relement='Au', NrDep='True', L=1.0, B=1.0,
                 H=1.0, norm=1.0, bkg=0.0, D=1.0, phi=0.1, U=-1.0, SF='None',Nphi=200,Npsi=400, tol=1e-3,
                 mpar={'Layers': {'Material': ['Au', 'H2O'], 'Density': [19.32, 1.0], 'SolDensity': [1.0, 1.0],
                                  'Rmoles': [1.0, 0.0], 'dL': [0.0, 0.0],'dLsig':[1.0,1.0],'dB':[0.0,0.0],'dBsig':[1.0,1.0]}}):
        """
        Documentation
        ...
        mpar        : Multi-parameter which defines the following including the solvent/bulk medium which is the last one. Default: 'H2O'
                        ...
                        dL and dB (Thicknesses in Angs) of the layers starting with 0 for the first layer as the first layer thickness provided by L,B, and H values
                        dLsig and dBsig (Width in Angs) of the distribution of dL and dB
        """
        if type(x)==list:
            self.x=np.array(x)
        else:
    ...
    @lru_cache(maxsize=10)
    def parallelopiped(self, q, L, Lsig, B, Bsig, H, rho, dist='Gaussian', Np=10, Nphi=200, Npsi=400,tol=1e-3):
        q = np.array(q)
        ...
        for i in range(Np):
            fft = parallelopiped_ml(q, Lt[i], Bt[i], H, rho, Nphi, Npsi)
    ...
    def update_params(self):
        ...
        key = 'dL'
        self.__dL__ = [self.params['__%s_%s_%03d' % (mkey, key, i)].value for i in range(Nmpar)]
        key = 'dB'
        self.__dB__ = [self.params['__%s_%s_%03d' % (mkey, key, i)].value for i in range(Nmpar)]
        key = 'dLsig'
        self.__dLsig__ = [self.params['__%s_%s_%03d' % (mkey, key, i)].value for i in range(Nmpar)]
        key = 'dBsig'
        self.__dBsig__ = [self.params['__%s_%s_%03d' % (mkey, key, i)].value for i in range(Nmpar)]
        key = 'Material'
    ...
    def y(self):
        ...
        self.update_params()
        self.__L__=np.array(self.__dL__)
        self.__B__=np.array(self.__dB__)
        self.__dLsig__ = np.array(self.__dLsig__)
        self.__dBsig__ = np.array(self.__dBsig__)
        self.__L__[0]=self.L
        self.__B__[0]=self.B
        if type(self.x) == dict:
            sqf = {}
            for key in self.x.keys():
                ...
                rho, eirho, adensity, rhor, eirhor, adensityr = calc_rho(R=tuple(self.__L__),
                                                                         ...
                                                                         NrDep=self.NrDep)
                sq, L, Ldist, B, Bdist=self.parallelopiped(tuple(self.x[key]), tuple(self.__L__), tuple(self.__dLsig__),
                                       tuple(self.__B__), tuple(self.__dBsig__),
                                       self.H, tuple(rho),
                                       dist = self.dist, Np = self.Np, Nphi = self.Nphi, Npsi = self.Npsi, tol=self.tol)
            ...
            sq, L, Ldist, B, Bdist = self.parallelopiped(tuple(self.x), tuple(self.__L__), tuple(self.__dLsig__),
                                                         tuple(self.__B__), tuple(self.__dBsig__),
                                                         self.H, tuple(rho),
            ...
            if not self.__fit__:
                if len(self.__L__)>2:
                    ...
                    self.output_params['L_1']={'x':L[:,0],'y':Ldist}

...
class Sphere_Uniform_Edep: #Please put the class name same as the function name
    def __init__(self, x=0, Np=20, error_factor=1, bkg=0.0,dist='Gaussian', relement='Au', Energy=None, NrDep='False', norm=1.0,
                 D=1.0, phi=0.1, U=-1.0, SF='None',Rsig=0.0, norm_err=0.01,normQ=0,Energy_Offset=0.0,
    ...
    def init_params(self):
        ...
        self.params.add('Energy_Offset',value=self.Energy_Offset,vary=0,min=-1e-4,max=20,expr=None,brute_step=1e-3)
    ...
    def y(self):
        ...
        if type(self.x) == dict:
            ...
            if not self.__fit__:
                ...
                self.output_params['adensity_r'] = {'x': adensityr[:, 0], 'y': adensityr[:, 1]*scale, 'names':['r (Angs)', 'adensity (Molar)']}
                self.output_params['cdensity_r'] = {'x': cdensityr[:, 0], 'y': cdensityr[:, 1], 'names':['r (Angs)', 'Density (g/cm<sup>3</sup>)']}
            sqf = self.output_params['Total']['y']
        return sqf

...
class Cylinder_Uniform_Edep: #Please put the class name same as the function name
    def __init__(self, x=0, Np=10, error_factor=1.0, dist='Gaussian', Energy=None, Energy_Offset=0.0, relement='Au', NrDep='False', H=1.0, HvvgtR=False,
                 Rsig=0.0, norm=1.0, norm_err=0.01, bkg=0.0, D=1.0, phi=0.1, U=-1.0, SF='None',Nalf=200,term='Total', normQ=0,
        ...
        Energy_Offset: Energy offset in keV
        ...
        bkg          : Constant incoherent background for SAXS-term
    ...
    def init_params(self):
        ...
        self.params.add('Energy_Offset', value=self.Energy_Offset, vary=0.0, min=-np.inf, max=np.inf, expr=None, brute_step=0.1)
        ...
        if type(self.x) == dict:
            ...
            for key in self.x.keys():
                ...
                rho, eirho, adensity, rhor, eirhor, adensityr, cdensityr = calc_rho(R=tuple(self.__R__),
                                                                                    ...
                                                                                    NrDep=self.NrDep)
                sqft=self.cylinder(tuple(self.x[key]), tuple(self.__R__), self.H, HvvgtR, self.Rsig,
                                        tuple(rho), tuple(eirho), tuple(adensity), dist=self.dist, Np=self.Np, Nalf=self.Nalf)
                if self.SF is None:
                ...
                sqf[key] = (self.norm * 1e-9 * 6.022e20 * sqft * struct + self.bkg)*self.x[key]**self.normQ
            ...
            tsqf = self.cylinder(tuple(self.x), tuple(self.__R__), self.H, HvvgtR, self.Rsig,
                                                     tuple(rho), tuple(eirho),
                                                      tuple(adensity), dist=self.dist, Np=self.Np, Nalf=self.Nalf)
            sqf = (self.norm*1e-9 * np.array(tsqf) * 6.022e20 * struct + self.bkg)*self.x**self.normQ  # in cm^-1 if not normalized by Q
...
if __name__=='__main__':
    x = {'Total_E@25.514': np.logspace(np.log10(0.003), np.log10(0.15), 500)}
    fun=Cylinder_Uniform_Edep(x=x)
    print(fun.y())
